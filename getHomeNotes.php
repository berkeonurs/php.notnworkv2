...
//            $notesList = $db->get('notes n');
            $sorgu = "SELECT notes.noteId,notes.userId,notes.noteTitle,notes.noteLesson,notes.noteDesc,notes.noteType,notes.noteDate,notes.noteTeacherListId,notes.noteTeacherName,notes.departmentId,notes.noteActive,notesimages.id,notesimages.imageUrl,notesimages.noteId,users.userName,users.userLastName,users.userPhoto FROM notes INNER JOIN notesimages ON notes.noteId = notesimages.noteId  INNER JOIN users ON notes.userId = users.id WHERE userId IN (SELECT userFollowed FROM usersfollow WHERE userId=$userId)";
